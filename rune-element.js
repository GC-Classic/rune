class RuneElement extends HTMLElement {
    ...
    createHTML = () => {
        ...
        this.shadowRoot.Q('figure').append(
    ...
    update = {
        pri: () => {
            this.shadowRoot.Q('.level').textContent = `+${this.rune.primary.level}`;
            this.rune.primary.level && this.classList.add('reinforced');
            this.title = this.rune.stringify();
        },
        sec: () => {
            this.rune.acquired.length > this.shadowRoot.Q('.bottom').children.length ?
                this.shadowRoot.Q('.bottom').append(E('prop-icon', {prop: this.rune.acquired.at(-1)})) :
                this.rune.secondary.forEach(s => this.shadowRoot.Q(`prop-icon[prop=${s.prop}]`).level = s.level);
            this.title = this.rune.stringify();
        },
        delta: () => {
            if (DEMO) return;
            this.calculate.delta();
            this.shadowRoot.Q('.delta').value = this.delta;
        },
        score: () => {
    ...
    action = {
        ...
        reinforce: () => {
            ...
            this.shadowRoot.Q('dialog').prepend(new ReinforceDialog(this));
            this.shadowRoot.Q('dialog').showModal();
        }
    }
    ...
    attributeChangedCallback() {
        this.shadowRoot.Q('figure').title = this.rune.dismantle;
    }
